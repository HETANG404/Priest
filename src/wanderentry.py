...
from core.wander import *  # 假设你封装了这几个函数
import sys
...
def print_menu():
    print("=" * 50)
    print(" Pixiv wandering ")
    print("=" * 50)
    print("1. 下载日周月而十八天梯，分享专用线路，生成pdf")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            pdf_path = os.path.join(BASE_PATH, "pdfs", f"merged_wander_{mode}_{page}p_{date_str}.pdf")

            for i in range(page):
                url = f"{base_url}ranking.php?mode={mode}&p={i + 1}&format=json"
                wander_crawler_ranking(url, i, path)
